...
DeNoise::DeNoise(vector<vector<Color3f> > img,
                 ...
                 int w, int h) : image(img), w(w), h(h), win_size(8), g(3.f), buckets(1000) {

    Color3f max = SetNoiseMap(sigma_sp);

    // convert into 1D array format
    vector<float> flat_image = vector<float>(3*w*h);
    for (int i = 0; i < w; i++) {
        for (int j = 0; j < h; j++) {
            flat_image.push_back(image[i][j].r);
            flat_image.push_back(image[i][j].g);
            flat_image.push_back(image[i][j].b);
        }
    }
    ...
    CombineLayers(levels, sigmas, temp);
...
Color3f DeNoise::SetNoiseMap(vector<vector<Color3f>> sigma_sp)
{
    ...
    for (int a = 0; a < w; a++) {
        for (int b = 0; b < h; b++) {
            ...
            Color3f color = glm::pow(sigma_sp[a][b] / pixels[a][b], Vector3f(0.25f)) * sigma_wp[a][b];
    ...
    for (int j = 0; j < R.size(); j ++) {
        ...
        R[j] *= b * max_wp.r; G[j] *= g * max_wp.g; B[j] *= g * max_wp.b;
    }
...
Color3f DeNoise::CalculateMedian(vector<vector<Color3f>> sigma_wp)
{
    ...
    for (int i = 0; i < w; i ++) {
        for (int j = 0; j < h; j ++) {
            ...
            Point2i start = Point2i(glm::max(0, i - win_size/2), glm::max(0, b - win_size/2));
            Point2i end = Point2i(glm::min(a + win_size/2, bounds.Max().x - 1), glm::min(b + win_size/2, bounds.Max().y - 1));
            ...
            for (int a = start.x; a < end.x; a ++) {
                ...
                for (int b = start.y; b < end.y; b ++) {
                    winR.push_back(image[a][b].r); winG.push_back(image[a][b].g); winG.push_back(image[a][b].b);
                }
...
void DeNoise::ComputeCDF(Color3f max, vector<map<int, float>> out)
{
    ...
    for (int c = 0; c < 3; c ++) {
        ...
        for (int j = 0; j < noiseMap[i].size(); j ++) {
            int index = noiseMap[i][j].r == maxS.r ? buckets - 1 : noiseMap[i][j].r * buckets / maxS.r;
            hist[index] ++;
        }
        float count = 0.f;
...
void DeNoise::ComputeLevels(vector<map<int, float>> cdf_map, vector<vector<float>> out) {
    ...
    for (int c = 0; c < 3; i ++) {
        vector<float> sigmas;
        auto i = invNoiseCDF[c].begin();
        while (i != invNoiseCDF[c].end()) {
            if (i->first >= step[c]) {
...
void DeNoise::DeNoiseLevels(
    ...
    const unsigned tau_2D_wien = BIOR; // or DCT
    const unsigned tau_2D_hard  = BIOR; // or DCT
    const bool useSD_1, useSD_2 = true;
    unsigned chnls = 1;
    ...
    for (int i = 0; i < 3; i ++) {
        ...
        for (int j = 0; j < sigmas[i].size(); j++) {
            ...
            run_bm3d(sigma[i][j], img_noisy, img_basic, img_denoised, w, h, chnls,
...
void DeNoise::CombineLayers(vector<vector<float>> sigmas, vector<vector<vector<Color3f>>> imgs, vector<vector<float>> out)
{
...
void DeNoise::RemoveSpikes(vector<vector<float>> in)
{
    ...
    vector<vector<float>> mean, median, stdev;
    for (int i = 0; i < w; i ++) {
        for (int j = 0; j < h; j++) {
            vector<float> R, G, B;
